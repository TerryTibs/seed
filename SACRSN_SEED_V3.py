# ==============================================================================
# SACRSN-SEED IMMORTAL CORE — THE UNIFIED EDITION (v3)
# ==============================================================================
#
# STATUS:  AUDITED × 2 · FIXED × 2 · ALL FEATURES WIRED · ALL ISSUES RESOLVED
#
# AUDIT PASS 2 FIXES (from SACRSN_SEED_FIXED.py audit):
#  [HIGH]     Removed duplicate ShardedIdentity.merge_gradients() from run_cycle
#             (was called twice: once in phase_evolve, once after — second call
#              overwrote freshly regenerated weights with stale pre-regen state)
#  [MEDIUM]   Kill-switch now fires AFTER EWC penalty — catches EWC-inflated divergence
#  [MEDIUM]   _prev_seeds is now per-agent dict — no cross-agent replay contamination
#  [MEDIUM]   reflect() now calls m.eval()/m.train() around generate() — Dropout off
#  [LOW]      generate() has n_paths param — reflect() uses n_paths=1 (no overhead)
#  [LOW]      CivilizationMind.get_context() wired into reflect() prompt
#  [LOW]      SyntheticBuffer.add() wired in reflect() and generate_demo()
#             (closes self-improvement loop: quality output → data_recursive.txt)
#  [LOW]      AutonomousResearch.propose() now records telemetry as hypothesis
#
...
class AutonomousResearch:
    ...
    def propose(self, telemetry):
        ...
        if telemetry:
            self.hypothesis = telemetry  # e.g. {"loss": float, "gen": int}
...
class ImmortalCoreController:
    ...
    def reflect(self):
        ...
        with torch.no_grad():
            ...
            # [FIX MEDIUM] eval mode — disables Dropout for clean generation
            m.eval()
            out = m.generate(enc, n_paths=1)  # [FIX LOW] n_paths=1 — no best-of-N overhead for reflection
            m.train()
            txt = self.data.decode(out[0].tolist())

            # [FIX LOW] SyntheticBuffer quality filter — good output feeds next training epoch
            if len(txt) > 20:
                # Estimate perplexity from cross-entropy on the generated text
                try:
    ...
    def generate_demo(self):
        if self.rank == 0:
            ...
            # [FIX LOW] Feed quality-filtered demo output back into training corpus
            if len(txt) > 20:
                try:
                    enc_gen = torch.tensor([self.data.encode(txt[:CONFIG["BLOCK_SIZE"]])], device=DEVICE)
                    if enc_gen.shape[1] > 1:
