# ==============================================================================
# SACRSN-SEED IMMORTAL CORE — THE UNIFIED EDITION (v4)
# ==============================================================================
#
# STATUS:  AUDITED × 3 · FIXED × 3 · ALL FEATURES WIRED · ALL ISSUES RESOLVED
#
# AUDIT PASS 3 FIXES + RUNTIME FIX (from SACRSN_SEED_V3.py audit + traceback):
#  [RUNTIME]  OOM crash in generate() — GodheadTransformer.forward() now accepts
#             world_sim override; generate() passes world_sim=1, eliminating the
#             INFERENCE_PATHS × WORLD_SIM = 4×5 = 20-sequence blowup per token step.
#             generate_demo() max_new reduced 300→150 for CPU headroom.
...
class ImmortalCoreController:
    ...
    def phase_train(self, generation):
        ...
        self.res.propose({"loss": self.prev_loss, "gen": generation})
        self.res.act(self)
